...
const publishableKey =
  "pk_test_51PdfJeCXxlVIqo4hm7ZyfaR4DOQpjvBKWonLDH4Uak5s0AcuMbk55p3wZjCaBOtdzaxJ9gBySx0m3ifVcQquDHIo00iXsWdZoG";
...
async function initializePaymentElement() {
  ...
  const { clientSecret, creationTimestamp } = await fetch(
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const redirectUrl = `http://localhost:3000/success?payment_intent_client_secret=${clientSecret}&creationTimestamp=${encodeURIComponent(
      creationTimestamp
    )}`;
